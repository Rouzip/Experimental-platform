...
class Mnist(DataSource):
    ...
    def __init__(self):
        ...
        num_train = int(m * data_split[0])
        num_test = int(m * data_split[1])
    ...
    def gen_dummy_non_iid_weights(self):
        ...
        num_classes_this_client = random.randint(
            1, Mnist.MAX_NUM_CLASSES_PER_CLIENT + 1)
        classes_this_client = random.sample(
            self.classes.tolist(), num_classes_this_client)
    ...
    def post_process(self, xi, yi):
        ...
        # keras通过使用别的框架来进行model解析，这里是Tensorflow
    ...
    def partitioned_by_rows(self, num_workers, test_reserve=0.3):
        ...
        return {
            ...
            "test": post_process(self.x[np.arange(n_train, n_train + n_test)],\
    ...
    def sample_single_non_iid(self, x, y, weight=None):
        ...
        candidates_idx = np.array(
            [i for i in range(y.shape[0]) if y[i] == chosen_class])
    ...
    def fake_non_iid_data(self, min_train=100, max_train=1000,
        ...
        train_set = [self.sample_single_non_iid(
            self.x_train, self.y_train,
            my_class_distr) for _ in range(train_size)]
        test_set = [self.sample_single_non_iid(
            self.x_test, self.y_test,
            my_class_distr) for _ in range(test_size)]
        valid_set = [self.sample_single_non_iid(
            self.x_valid, self.y_valid,
            my_class_distr) for _ in range(valid_size)]
