...
class GlobalModel(object):
    ...
    def aggregate_loss_accuracy(self, client_losses,
        ...
        # 改用向量写法更快？
        # aggr_loss = np.sum(
        #     client_losses[:len(client_sizes)]/total_size*client_sizes[:len(client_sizes)])
        aggr_accuraries = np.sum(client_accuracies[i] / total_size * client_sizes[i]
                                 for i in range(len(client_sizes)))
        # aggr_accuraries = np.sum(client_accuracies[:len(
        #     client_sizes)]/total_size*client_sizes[:len(client_sizes)])
...
class FLServer(object):
    ...
    def register_handles(self):
        ...
        def handle_client_ready(data):
            ...
            if len(self.ready_client_sids) >= FLServer.MIN_NUM_WORKERS \
        ...
        def handle_client_update(data):
            ...
            for x in data:
                # if data is dict,for x in data.keys()? or in items()?
                if x != 'weights':
                    print(x, data[x])
            # data:
            #   weights
            ...
            if data['round_number'] == self.current_round:
                ...
                self.current_round_client_updates[-1][
                    'weights'] = pickle_string_to_obj(data['weights'])
        ...
        def handle_client_eval(data):
            ...
            if len(self.eval_client_updates) > FLServer.NUM_CLIENTS_CONTACTED_PER_ROUND * .7:
                ...
                print("== done ==")
                # special value, forbid evaling again
                self.eval_client_updates = None

    # Note: we assume that during training the #workers will be >=
    # MIN_NUM_WORKERS
    def train_next_round(self):
        ...
        client_sids_selected = random.sample(
            list(self.ready_client_sids),
...
def obj_to_pickle_string(x):
    ...
    # TODO: compare pickle vs msgpack vs json for serialization; tradeoff:
    # computation vs network IO
